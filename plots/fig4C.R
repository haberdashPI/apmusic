...
df = read.csv(file.path("preprocessing","data",fig5C_data))
model = read.csv(file.path("preprocessing","data",fig5C_model))
...
pl = pl + geom_smooth(stat="identity",data=model,color="black",fill="lightgray",
                      aes(y=mean*100,ymin=lower*100,ymax=upper*100,
                          linetype=stimulus_label))
pl = pl + geom_point(data=df,aes(shape=stimulus_label,fill=stimulus_label),
                     size=1.5)
...
file = file.path("plots","pdfs",paste("fig5C_",Sys.Date(),".pdf",sep=""))
