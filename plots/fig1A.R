...
rmodel = read.csv(file.path("preprocessing","data",fig1A_model))
rdata = read.csv(file.path("preprocessing","data",fig1A_data))

pl = ggplot(rmodel,aes(y=mean,x=standard_label))
...
file = file.path('plots','pdfs',paste('fig1A_',Sys.Date(),'.pdf',sep=''))
