...
rdata = read.csv(file.path("preprocessing","data",fig1B_data))
rmodel = read.csv(file.path("preprocessing","data",fig1B_model))

p = function(xf,yf){
  ## rdata['ymin'] = rdata[paste('mean',yf,sep='_')] -
  ##   rdata[paste('std',yf,sep='_')]
  ## rdata['ymax'] = rdata[paste('mean',yf,sep='_')] +
  ##   rdata[paste('std',yf,sep='_')]
  ## rdata['xmin'] = rdata[paste('mean',xf,sep='_')] -
  ##   rdata[paste('std',xf,sep='_')]
  ## rdata['xmax'] = rdata[paste('mean',xf,sep='_')] +
  ##   rdata[paste('std',xf,sep='_')]
...
file = file.path('plots','pdfs',paste('fig1B_',Sys.Date(),'.pdf',sep=''))
