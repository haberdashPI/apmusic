...
df = read.csv(file.path("preprocessing","data",fig5B_data))
model = read.csv(file.path("preprocessing","data",fig5B_model))

pl = ggplot(df,aes(x=posttest*100,y=mean*100))
pl = pl + geom_smooth(stat="identity",data=model,
                      aes(y=mean*100,ymin=100 * (mean-SE),ymax=100 * (mean+SE)),
                      color="black")
...
file = file.path("plots","pdfs",paste("fig5B_",Sys.Date(),".pdf",sep=""))
