...
# the linear model is unusued in this project, so it is commented
# out for clarity

# linear_model = sv.load_model('linear',use_package_cache=True)
robit_model = sv.load_model(op.join('util','robit'))
robit2_model = sv.load_model(op.join('util','robit2'))
...
#                                 **sample_kws)
#     if cache_file:
#       sv.write_samples(fit.extract(),cache_file)
#   else:
#     fit = sv.read_samples(cache_file)
...
class BaseRegressResults(object):
  ...
  def cache(self,cache_file):
    if issubclass(self.fit,pystan.StanFit4Model):
      sv.write_samples(self.fit.extract(),cache_file)
    else: sv.write_samples(fit,cache_file)
