...
d = c.groupby(['condition','regimen']).meanpre.mean().reset_index()
p = model.predict(d,use_dataframe=True)
regs = p.set_index(['regimen','condition','sample']).y.unstack(level='regimen')*100

print "----------------------------------------"
print "Difference in % improvement from trained to untrained stimulus (by regimen)"
print ss.coef_table(regs.query('condition == "4th"').values - regs.query('condition == "4th_gen"').values,regs.columns)

print "----------------------------------------"
print "% improvement differences by regimen for the untrained stimlus (triangle tone)"
