...
model_reg_samples = op.join("preprocessing","data",
                            "fig5C_samples_byregimen_"+dt+".h5")
...
d = pd.DataFrame([{'posttest': posttest,
                   'pretest': model_foil.df.pretest.mean(),
                   'stimulus_label': stimulus_label}
                   for posttest in np.linspace(0.4,1,50)
                   for stimulus_label in (model_foil.df.
                                          stimulus_label.unique())])
predict = (model_foil.predict(d,use_dataframe=True).
...
model_file = op.join("preprocessing","data","fig5C_model_"+dt+".csv")
