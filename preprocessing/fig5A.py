...
model_samples = op.join("preprocessing", "data", "fig5A_samples_"+dt+".h5")
...
predict = (p.groupby(['regimen']).y.apply(ss.coef_stats_ns).
           unstack(level=1).reset_index())

data_file = op.join("preprocessing", "data", "fig5A_data_"+dt+".csv")
lc.to_csv(data_file, header=True)
print "Created "+data_file

model_file = op.join("preprocessing", "data", "fig5A_model_"+dt+".csv")
