...
model_samples = op.join("preprocessing","data","fig4B_samples_"+dt+".h5")
...
model_reg_samples = op.join("preprocessing","data",
                            "fig4B_samples_byregimen_"+dt+".h5")
...
d = pd.DataFrame({'trained': np.linspace(0.4,1,50),
                  'meanpre': model.df.meanpre.mean()})
rmodel = (model.predict(d,use_dataframe=True).groupby('trained').y.
          apply(ss.coef_stats_ns).unstack(level=1).reset_index())
rdata = model.df

data_file = op.join("preprocessing","data","fig4B_data_"+dt+".csv")
rdata.to_csv(data_file,header=True)
print "Created "+data_file

model_file = op.join("preprocessing","data","fig4B_model_"+dt+".csv")
